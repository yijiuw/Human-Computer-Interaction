"""-------开发日志-------
时间：4/15
内容：在4.0_sd_sv_Qwen_edgeTTS.py的基础改成视频交互
目标：1.同上
改变：1.添加了start_camera()函数
     2.添加了stop_camera()函数
     3.添加了camera_loop()函数
     4.在start()函数中添加了摄像头的相关内容
问题：1.opencv似乎不能用在子线程中 这个问题将再1.6_test_opencv_thread.py中测试
"""
...
if __name__ == "__main__":
    ...
    gesture_thread = threading.Thread(target=recorder.camera_loop)
    gesture_thread.daemon = True  # 设为守护线程，主线程结束时自动退出
    gesture_thread.start()
    print("摄像头线程状态:", gesture_thread.is_alive())
    print("线程ID:", gesture_thread.ident)  # 不是None表示已启动
    # 主线程可以继续执行其他任务
    print("主线程正在运行...")
    while True:
        time.sleep(1)
