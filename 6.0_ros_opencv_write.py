"""-------开发日志-------
时间：4/12
内容：在5.4_opencv_write.py的基础上加入了0.12_learn_kinect2_MediaPipe.py的内容
目标：1.从ros节点中读取
问题：
"""
...
class GestureDetector:
    def __init__(self):
        ...
        # 订阅Kinect图像话题
        rospy.Subscriber("/kinect2/hd/image_color", Image, self.image_callback)

    def image_callback(self, msg):
        """ROS图像回调函数"""
        try:
            # 将ROS Image消息转为OpenCV格式
            ...
            self.cv_image = np.frombuffer(msg.data, dtype=dtype).copy()
            self.cv_image = self.cv_image.reshape(msg.height, msg.width, channels)
    ...
    def start(self):
        ...
        while not rospy.is_shutdown():
            if self.image_received and self.cv_image is not None:
                # 检测手势
                results, frame = self.identify_hand(self.cv_image)
