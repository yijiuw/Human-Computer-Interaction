"""-------开发日志-------
时间：4/9
内容：在3.1_Sd_sv_edgeTTs.py的基础上添加Qwen来对话
目标：1.添加Qwen函数，用来回复输入的内容
"""
...
class VoiceRecorder:
    def __init__(self):
        ...
        # 系统提示设置
        self.system_prompt = "你只会用中文回答，不要超过50字"
    ...
    def read_gesture():
        try:
            with open("gesture_data.txt", "r") as f:
                return f.read().strip()
    ...
    def respond(self,text) -> str:
        """使用Qwen大模型生成回复"""
        #读取文件里的信息
        gesture_text = self.read_gesture()

        self.conversation_history.append({"role": "user", "content": f"当我问你视频里的手势是什么的时候，你一定要回答：{gesture_text}"})
